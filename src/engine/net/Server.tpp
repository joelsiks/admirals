...
template <typename T>
Server<T>::Server(uint16_t port) : acceptor(io_context, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port)) {}
    ...
    acceptor.async_accept(
        [this](std::error_code ec, asio::ip::tcp::socket socket) {
            if (!ec) {
                ...
                std::shared_ptr<Connection<T>> new_connection = std::make_shared<Connection<T>>(Connection<T>::Owner::SERVER, io_context, std::move(socket), incoming_messages, this, &Server<T>::OnClientValidated);
...
template <typename T>
void Server<T>::MessageClient(std::shared_ptr<Connection<T>> client, const Message<T>& message) {
...
template <typename T>
void Server<T>::MessageAllClients(const Message<T>& message, std::shared_ptr<Connection<T>> ignore_client) {
    ...
    if (client_disconnected) {
        ...
        // Reorders the vector so that all bad clients are at the end, then erases them
