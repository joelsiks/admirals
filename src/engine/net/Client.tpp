...
bool Client<T>::Connect(const std::string& host, const std::string& port) {
    try {
        ...
        m_connection = std::make_unique<Connection<T>>(Connection<T>::Owner::CLIENT, m_io_context, asio::ip::tcp::socket(m_io_context), m_incoming_messages);
...
template <typename T>
MessageQueue<OwnedMessage<T>>& Client<T>::Incoming() {
