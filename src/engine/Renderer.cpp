...
void Renderer::drawTexture(const VK2DTexture texture, const Vector2 &position,
                           const Vector2 &orgSize, const Vector2 &drawSize) {
    Vector2 scale =
        Vector2(drawSize.x() / orgSize.x(), drawSize.y() / orgSize.y());
    vk2dRendererDrawTexture(texture, position.x(), position.y(), scale.x(),
                            scale.y(), 0, 0, 0, 0, 0, orgSize.x(), orgSize.y());
}
